...
export default {
  async fetch(request, env) {
    ...
    // ── MFA (worker-side TOTP) ───────────────────────────────────
    // All five routes verify the caller's Firebase ID token first.
    // Per-IP rate-limited via checkPublicRateLimit (mfa_status,
    // mfa_enroll, mfa_verify scopes). Per-UID lockout: 5 wrong
    // codes → 30 min lockout (KV-backed).
...
const PUBLIC_RATE_MAX = {
  ...
  mfa_status: 30,    // /mfa/status — cheap, called on every page load
  mfa_enroll: 5,     // /mfa/enroll-start — expensive (Firebase write); enroll is rare
  mfa_verify: 10,    // /mfa/verify, /mfa/enroll-confirm, /mfa/unenroll — UID lockout is the harder cap
  default: 30,
};
...
// ════════════════════════════════════════════════════════════════
// Worker-side TOTP MFA (RFC 6238)
// ════════════════════════════════════════════════════════════════
//
// Why this exists: Firebase Auth's built-in TOTP MFA requires the
// Blaze (pay-as-you-go) plan. We're on Spark. So we implement TOTP
// inside the worker:
//
//   • Secret (20 random bytes, base32) lives in KV (`mfa_totp_<uid>`).
//     The client receives it ONCE during enrollment to set up their
//     authenticator app; from then on the secret is server-only.
//
//   • Code verification uses Web Crypto HMAC-SHA1 per RFC 6238 with
//     a 30-second window. We accept the previous + current + next
//     windows (±30s clock skew). Replay defense: each successful
//     verification records the consumed counter; the same counter
//     cannot be re-used for ~3 windows.
//
//   • After a successful /mfa/verify, the worker sets a Firebase
//     custom claim `mfaVerifiedAt: <unix seconds>` on the user's
//     account. Firestore rules can require this claim to be recent
//     (<12h old) for sensitive collections. After enrollment, the
//     `mfaEnabled: true` claim is set.
//
//   • Rate limited: 5 wrong codes per UID per 10 minutes locks the
//     account out for 30 min. Stored in KV `mfa_lock_<uid>`.
//
// ID tokens: we verify them by calling Google's identitytoolkit
// `accounts:lookup` with our service-account access token. Google
// rejects invalid/expired tokens and returns the user record for
// valid ones — same behaviour as Firebase Admin SDK's verifyIdToken
// without us having to load the JWKS and verify RS256 ourselves.
...
const MFA_VERIFIED_VALID_S  = 12 * 60 * 60;  // mfaVerifiedAt valid for 12h
const MFA_STEP_SECONDS      = 30;            // RFC 6238 standard
const MFA_WINDOW            = 1;             // ±1 step (±30s) tolerance

// Base32 (RFC 4648, no padding for otpauth: URIs). Used for the
// authenticator-app secret display + the otpauth:// URL.
...
async function setUserCustomClaims(env, uid, patch) {
  // Re-fetch existing claims first so we don't clobber unrelated ones.
...
// Lockout state per UID. Returns { locked: bool, until?: number,
// fails: number }.
...
// Shared response helper for MFA endpoints: writes an audit-log
// breadcrumb to console (picked up by `wrangler tail`) and returns
// JSON.
function mfaResponse(action, uid, payload, request, status) {
  console.log('[mfa]', action, uid ? uid.slice(0, 8) : '?', JSON.stringify(payload).slice(0, 200));
  return jsonResponse(payload, request, status || 200);
}

// POST /mfa/status
// Body: { idToken }
// Returns: { enrolled, locked, lockUntil?, verifiedAt? }
...
// Generates a fresh 20-byte secret, stores it in pending KV with
// 10-min TTL. Client shows QR + secret to the user, they type the
// 6-digit code into /mfa/enroll-confirm. Replaces any existing
// pending secret for this UID.
async function handleMfaEnrollStart(request, env) {
  ...
  // out of MFA accidentally).
  const existing = await env.KV_NAMESPACE.get(MFA_KV_SECRET_PREFIX + user.uid);
  if (existing) {
    return jsonResponse({ error: 'already_enrolled' }, request, 409);
  }
  ...
  await env.KV_NAMESPACE.put(
    ...
    JSON.stringify({ secret, createdAt: Math.floor(Date.now() / 1000) }),
...
// Body: { idToken, code }
// Verifies the user can compute the right code from the pending
// secret. If yes, promotes pending → permanent and sets the
// `mfaEnabled: true` custom claim. Client must then call
// auth.currentUser.getIdToken(true) to pick up the new claim.
async function handleMfaEnrollConfirm(request, env) {
  ...
  const verify = await totpVerify(pending.secret, code);
  if (!verify.ok) {
    return jsonResponse({ error: 'invalid_code' }, request, 400);
  }

  const now = Math.floor(Date.now() / 1000);
  await env.KV_NAMESPACE.put(
    MFA_KV_SECRET_PREFIX + user.uid,
    JSON.stringify({ secret: pending.secret, enrolledAt: now })
  );
  ...
  try {
    await setUserCustomClaims(env, user.uid, {
      mfaEnabled: true,
      mfaEnrolledAt: now,
      mfaVerifiedAt: now,  // they just verified, count it
    });
  } catch (e) {
    // Claim write failed but secret was saved. Surface the error —
    // the user can retry the enroll-confirm and we'll re-merge the
    // claim (no harm: setUserCustomClaims is idempotent).
    return jsonResponse({ error: 'claim_write_failed', detail: String(e.message || e) }, request, 500);
  }

  return mfaResponse('enroll_confirm', user.uid, { enrolled: true, enrolledAt: now }, request);
}

// POST /mfa/verify
// Body: { idToken, code }
// Used on sign-in after Firebase Auth succeeds for accounts that
// have `mfaEnabled: true`. Sets `mfaVerifiedAt: <now>` so Firestore
// rules can require recent verification.
async function handleMfaVerify(request, env) {
  ...
  const code = String(body.code || '').trim();
  const verify = await totpVerify(stored.secret, code);
  if (!verify.ok) {
  ...
  if (await isReplayed(env, user.uid, verify.counter)) {
    await recordMfaFail(env, user.uid);
    return jsonResponse({ error: 'code_already_used' }, request, 400);
  }
  await markReplayUsed(env, user.uid, verify.counter);
  ...
  return mfaResponse('verify', user.uid, {
    verified: true,
    verifiedAt: now,
...
// disable MFA). Clears the secret + the mfaEnabled claim.
async function handleMfaUnenroll(request, env) {
  ...
  const lock = await getMfaLockState(env, user.uid);
  if (lock.locked) {
    return jsonResponse({ error: 'mfa_locked', lockUntil: lock.until }, request, 429);
  }
  ...
  return mfaResponse('unenroll', user.uid, { unenrolled: true }, request);
}
